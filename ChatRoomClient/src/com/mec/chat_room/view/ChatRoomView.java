...
public class ChatRoomView implements IMecView {
		...
		jbtnSend.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (speakMessage.length() <= 0) {
					ViewTool.showError(jfrmChatRoomView, "不能发送空消息！");
					return;
				}
				...
				if (!curFriend.getNick().equals(orgNick)) {
					ViewTool.showError(jfrmChatRoomView, "好友[" + orgNick + "]已经不在聊天室！");
					return;
				}
	...
	class ChatRoomAction extends ClientActionAdapter {
		...
		public boolean confirmOffline() {
			int choice = ViewTool.getChoice(jfrmChatRoomView, "是否下线？", JOptionPane.YES_NO_OPTION);
		...
		public void serverAbnormalDrop() {
			ViewTool.showError(jfrmChatRoomView, "服务器异常宕机，服务停止！");
			try {
				exitView();
			} catch (FrameIsNullException e) {
				e.printStackTrace();
		...
		public void dealToOther(String sourceId, String message) {
			...
			case I_AM_GONE:
				...
				if (curFriend.equals(leaveFriend)) {
					...
					ViewTool.showWarnning(jfrmChatRoomView, "好友[" + leaveFriend + "]已经离开");
				}
				break;
			case TO_OTHER:
				ArgumentMaker argument = new ArgumentMaker(parameter);
		...
		public void serverForcedown() {
			ViewTool.showMessage(jfrmChatRoomView, "服务器强制宕机，服务停止！");
			try {
				exitView();
			} catch (FrameIsNullException e) {
				e.printStackTrace();
	...
	@Override
	public void init() {
		jfrmChatRoomView = new JFrame("微易码2019年秋季聊天室");
		...
		JLabel jlblTopic = new JLabel("微易码聊天室", JLabel.CENTER);
